"""SparkSave: compact, dependency-free project context storage.
...
def estimate_tokens(text: str) -> int:
    ...
    This is intentionally approximate. It is useful for comparing before/after
    context size, not for billing or exact model-token accounting.
    """
    if not text.strip():
        return 0
    # A conservative mixed-text estimate: whitespace-separated pieces plus a
    # small punctuation/fragment allowance.
...
def compact_text(text: str) -> str:
    ...
    # Merge adjacent short continuation lines. Keep code-like lines untouched.
    merged: list[str] = []
    for line in lines:
...
def load_memory(path: str | Path) -> Memory:
    ...
    return Memory(**{field: data.get(field, []) if field != "project" and field != "goal" else data.get(field, "")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compact text for SparkSave.")
